...
// "16:40" IST → "11:10" UTC
  ...
  const handleJoinSession = async (e, bookingId) => {
    ...
    try {
      const result = await dispatch(getCallInfo({ bookingId, isAdmin: false })).unwrap();
...
const from = `${String(hour).padStart(2, "0")}:${String(m).padStart(2, "0")} ${ampm}`;
  alert(`Session abhi join nahi ho sakta. Join window starts at: ${from}`);
  return;
}
                ...
                {filteredBookings?.map((booking) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="session-date">{formatDate(booking.startAt, t)}</div>
                      <div className={`session-time${!(isPending || isAssigned) ? " dark" : ""}`}>
                        {formatTime(booking.startAt)}
                      </div>
                      ...
                      <div className="session-meta">
                        ...
                        <div className="meta-item">
                          {booking.mode === "video" ? <IconVideo /> : <IconPhone />}
                          {booking.mode === "video" ? t("videoCall") : t("voiceCall")}
                        </div>
                      </div>
