...
export function useAgoraCall({
  ...
  //   }
  // }, [bookingId, isAdmin, dispatch]);
  // ─── Leave Call (just leave, no API call) ─────────────────────────────────
  const leaveCall = useCallback(async () => {
    try {
      const { audioTrack, videoTrack } = localTracksRef.current;
      audioTrack?.close();
  ...
  // ─── End Call (leave + API call) ─────────────────────────────────────────
  const endAndLeaveCall = useCallback(async () => {
    try {
      const { audioTrack, videoTrack } = localTracksRef.current;
      audioTrack?.close();
      videoTrack?.close();
      ...
      // API call — mark as ended
      await dispatch(endCall({ bookingId, isAdmin })).unwrap();

      dispatch(clearTokenData());
      dispatch(setInCall(false));
      console.log("[Agora] Ended and left channel");
    } catch (err) {
      console.error("[Agora] End failed:", err);
    }
  }, [bookingId, isAdmin, dispatch]);
  // ─── Toggle Mic ─────────────────────────────────────────────────────────────
